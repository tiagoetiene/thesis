...
\caption{\label{fig:verification-procedure} Our verification procedure
  ...
  of one of three sampling parameters.%\myspace 
  }
\end{figure}
%
...
% Change from the beggining of the section: $\tau_j = \tau(j d)
% Same thing for: $\c_k = \c(k d)
...
\caption{\label{fig:stepsize_refinement} Step size refinement. The
  figure shows an isosurface of a trilinear function defined on the
  volume.%\myspace
  }
\end{figure}
%
...
\ref{fig:stepsize_refinement}).  
%We first generate a sequence of images $I_i$ using 
%a step size of $d_i = 1 / 2^i$.  
%progressively smaller step size.
%We then evaluate
%the error $E_i$.  
Equation~\eqref{eq:volume-rendering-equation} is commonly
discretized using traditional Riemann summation for numerical integration:
\begin{equation}
\int_0^D f(s(\mathbf{x}(\lambda))) \mathrm{d}\lambda =
\sum_{i=0}^{n-1} f(s(\mathbf{x}(i d))) d + O(d),
\label{eq:rectangle-rule}
\end{equation}
where $n$ is the number of subintervals and $d = D  /
...
For instance, the Trapezoidal Rule is a 2nd order method on the integral
of $f$. 
...
The interplay between the approximation errors of the inner and outer integrals is nontrivial; 
...
%   scalar field defined at different resolutions. The
...
%purposes because in this work we are not  interested in the quality of the 
...
% piecewise trilinear field provides us such a property). Moreover, the software packages tested in this work, 
...
%(step size, grid size, or pixel size) 
...
\subsection{Numerical Errors when the True Solution Is Unknown}
...
%nicely recovers the nonconvergence trend, as can be seen in
...
\begin{algorithm}[b]
\begin{codebox}
\Procname{$\proc{Verification Procedure}(G,\tau(s),d_0,N_0,h_0,\rho)$}
...
\li $F_0 \gets$ \proc{VolumeRendering($G,\tau(s),d_0,N_0,h_0$)}
\li \For $i \gets 1$ \To $\#$tests
\li     \Do \proc{Refine}($d_i, N_i, \text{ or } h_i$ depending on $\rho$)
\li         $F_i \gets$ \proc{VolumeRendering($G,\tau(s),d_i,N_i,h_i$)}
...
%Table \ref{tab:summary} indicates
...
\caption{\label{fig:fp-rcm-example} The figure shows images rendered
  ...
  %Step size convergence: Expected $k=1$. Before $k=1.2$.  After $k=1.2$.
...
\begin{figure}[t]
\centering 
\subfigure[Dataset refinement. Before, a linear trend is observed. After fixing an issue, a constant error is obtained.]{
  \includegraphics[width=0.9\linewidth]{chapter5/figures/problem-example-02.png}
  \label{fig:rcm-dataset}}
\subfigure[Pixel size refinement. Exp.: $k = 1$. Before:
  $k = 0.37$. After: $k = 1.23$]{
  \includegraphics[width=0.9\linewidth]{chapter5/figures/problem-example-03.png}
  \label{fig:rcm-pixel}}
