...
%
%\textcolor{red}{a referencia 15 � o artigo A survey of the marching cubes algorithm, talvez fosse melhor mencionar o case 7 onde existe a possibilidade de dois pontos criticos e fazer a referencia para o artigo do Hamish Carr}.  
%
...
\begin{figure}
    ...
    \includegraphics[width=0.98\linewidth]{chapter4/figures/executable.png}
     ...
     %In this particular case, we show a volume rendering image of the scalar field inside a single voxel. The user can interact with it and even change parameters, such as transfer functions. In this image, blue is used to encode  $f(x) < - \epsilon$, red  encodes $f(x) > \epsilon$ and yellow $-\epsilon \leq f(x) \leq \epsilon$.
     }}
...
%One possible way to solve this is by evaluating the signs of the critical points of the trilinear interpolant (blue dots in Figure \ref{fig:case13}): if the critical points are positive, then the tunnel is facing the positive vertex at the cube diagonal, whereas the negative vertex is completely isolated from all negative vertices, and vice-versa.
%
